...
class LeaveApprovalListUnnatiFragment : BaseFragment() {
        ...
        LeaveRequestAdapter(
            ...
            onApproveClick = { item ->
                showConfirmationDialog(
                    ...
                    item.LeaveRequestId?.let {
                        targetOutStandingViewModel.viewLeaveListApprovalUpdateUnnati(
    ...
    private fun initApiCall() = with(binding) {
        targetOutStandingViewModel.viewLeaveListApprovalUnnati(
    ...
    private fun manageToolBar() = with(binding) {
        ...
        (activity as HomeActivity).apply {
            ...
            manageToolBarTitle("Leave Approval")
    ...
    private fun observeViewLeaveList() {
        targetOutStandingViewModel.viewLeaveListStateApproval.observe(viewLifecycleOwner) { state ->
            when (state) {
        ...
        targetOutStandingViewModel.leaveApprovalUpdateState.observe(viewLifecycleOwner) { state ->
                when (state) {
                    is LeaveApprovalUpdateState.Loading -> showLoader()
                    is LeaveApprovalUpdateState.Success -> {
                        hideLoader()
                        showToast(state.response.message)
                        initApiCall()
                    }
                    is LeaveApprovalUpdateState.ApiError -> {
                        hideLoader()
                        showToast(state.message)
                    }

                    is LeaveApprovalUpdateState.NetworkError -> {
                        hideLoader()
                        showToast(state.message)
                    }
